...
class SimpananSukarelaScreen extends StatelessWidget {
  final Map<String, dynamic>? dashboardData;

  const SimpananSukarelaScreen({super.key, this.dashboardData});
  ...
  Widget build(BuildContext context) {
    // Cari rekening Simpanan Sukarela
    Map<String, dynamic>? rekeningSukarela;
    if (dashboardData != null && dashboardData!['rekening'] != null) {
      for (var rek in dashboardData!['rekening']) {
        if ((rek['produk'] as String).toLowerCase().contains('sukarela')) {
          rekeningSukarela = rek;
          break;
        }
      }
    }

    final double saldo = double.tryParse(rekeningSukarela?['saldo']?.toString() ?? '0') ?? 0;

    // Filter transaksi Simpanan Sukarela
    List<dynamic> history = [];
    if (dashboardData != null && dashboardData!['recent_transactions'] != null) {
      for (var trx in dashboardData!['recent_transactions']) {
        if ((trx['produk'] as String).toLowerCase().contains('sukarela')) {
          history.add(trx);
        }
      }
    }
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  Text(
                    'Simpanan bebas yang dapat disetor maupun ditarik kapan saja sesuai ketentuan Koperasi.',
                    style: GoogleFonts.inter(
                      color: Colors.white.withValues(alpha: 0.9),
                      fontSize: 10,
                      height: 1.4,
