    ...
    <div class="p-8 max-w-[1600px] mx-auto space-y-6">
        ...
        <section class="flex items-center justify-between">
            <div>
                ...
                <p class="text-slate-500 text-sm mt-1">Import transaksi simpanan dari file CSV</p>
            </div>
...
REK-SS-ANG-0002,penarikan,200000,Penarikan tunai</pre>
            ...
            <section class="bg-white rounded-xl shadow-sm p-6">
                <form action="{{ route('simpanan.upload.process') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div>
                        <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">File CSV <span class="text-danger">*</span></label>
                        <div class="border-2 border-dashed border-slate-200 rounded-xl p-8 text-center hover:border-primary/50 transition-colors">
                            ...
                            <input type="file" name="file" accept=".csv,.txt" required class="text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-5 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-primary/10 file:text-primary hover:file:bg-primary/20 file:transition-all file:cursor-pointer">
                            <p class="text-xs text-slate-400 mt-2">Format .csv atau .txt, maksimal 10MB</p>
                        </div>
                    </div>
