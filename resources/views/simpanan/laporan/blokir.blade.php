<x-app-layout>
    <div class="p-8 max-w-[1600px] mx-auto space-y-6">
        ...
        <section class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold font-headline tracking-tight text-blue-900">Laporan Rekening Blokir</h2>
                <p class="text-slate-500 text-sm mt-1">Daftar rekening simpanan yang sedang diblokir</p>
            </div>
                ...
                <table class="w-full text-left">
                    <thead>
                        <tr class="bg-slate-50/80 text-[11px] uppercase tracking-wider text-slate-500 font-bold">
                            ...
                            <th class="px-6 py-4">Tanggal Blokir</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50">
                        @forelse($rekening as $i => $rek)
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-3 text-sm text-slate-500">{{ $rek->updated_at?->format('d M Y H:i') }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-16 text-center">
                                    <span class="material-symbols-outlined text-[48px] text-slate-300 mb-3 block">lock</span>
                                    <p class="text-sm text-slate-400 font-medium">Tidak ada rekening yang diblokir</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
