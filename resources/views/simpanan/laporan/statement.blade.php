<x-app-layout>
    <div class="p-8 max-w-[1600px] mx-auto space-y-6">
        ...
        <section class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold font-headline tracking-tight text-blue-900">Statement Rekening</h2>
            ...
            <div class="flex gap-2">
                ...
                <a href="{{ route('simpanan.pdf.statement', [$rekening->id, 'from' => request('from'), 'to' => request('to')]) }}" target="_blank" class="flex items-center gap-2 px-4 py-2.5 bg-danger text-white text-sm font-semibold rounded-xl hover:bg-red-700 transition-all no-print">
                ...
                <a href="{{ route('simpanan.rekening') }}" class="flex items-center gap-2 px-4 py-2.5 bg-white border border-slate-200 text-slate-600 text-sm font-semibold rounded-xl hover:bg-slate-50 transition-all no-print">
        ...
        <section class="bg-white rounded-xl p-6 shadow-sm border-l-4 border-primary flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="flex items-center gap-4">
                <div class="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center text-primary font-bold text-lg">
                    {{ strtoupper(substr($rekening->anggota?->nama_lengkap ?? '?', 0, 2)) }}
                </div>
                <div>
                    <h3 class="text-lg font-bold text-blue-900 font-headline">{{ $rekening->anggota?->nama_lengkap }}</h3>
                    <p class="text-sm text-slate-500 font-data">{{ $rekening->no_rekening }} &middot; {{ $rekening->produk?->nama }} &middot; {{ $rekening->anggota?->no_anggota }}</p>
                </div>
            </div>
            <div class="text-right">
                <p class="text-[11px] text-slate-400 uppercase font-bold tracking-wider">Saldo Saat Ini</p>
                <p class="text-2xl font-bold font-data text-primary mt-1">Rp {{ number_format($rekening->saldo, 0, ',', '.') }}</p>
            </div>
        </section>
