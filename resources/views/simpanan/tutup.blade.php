<x-app-layout>
    <div class="p-8 max-w-[1600px] mx-auto space-y-6">
        ...
        <section class="flex items-center justify-between">
            <div>
                <h2 class="text-2xl font-bold font-headline tracking-tight text-blue-900">Tutup Rekening</h2>
                <p class="text-slate-500 text-sm mt-1">Penutupan permanen rekening simpanan anggota</p>
            </div>
        ...
        <div class="max-w-2xl space-y-6">
            ...
            <!-- Rekening Info -->
            <section class="bg-white rounded-xl shadow-sm p-6 border-l-4 border-danger">
                <div class="flex items-start gap-4">
                    <div class="p-3 bg-danger/10 rounded-xl">
                        <span class="material-symbols-outlined text-danger text-[24px]">cancel</span>
                    </div>
                    <div>
                        ...
                        <p class="text-sm mt-2">
                            Saldo saat ini:
                            <strong class="font-data {{ $rekening->saldo > 0 ? 'text-danger' : 'text-secondary' }}">Rp {{ number_format($rekening->saldo, 0, ',', '.') }}</strong>
                        </p>
                        @if($rekening->saldo > 0)
                            <p class="text-xs text-danger mt-1 font-medium">Rekening harus memiliki saldo Rp 0 sebelum ditutup. Tarik terlebih dahulu.</p>
                        @endif
                    </div>
                </div>
            </section>
