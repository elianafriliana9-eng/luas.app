<x-app-layout>
    <div class="p-8 max-w-[1600px] mx-auto space-y-6">
        ...
        <section class="flex flex-col sm:flex-row sm:justify-between items-start sm:items-end gap-4">
            <div>
                <h2 class="text-2xl font-bold font-headline tracking-tight text-blue-900">Riwayat Transaksi Simpanan</h2>
                <p class="text-slate-500 text-sm mt-1">Seluruh riwayat setoran, penarikan, dan pemindahbukuan</p>
            </div>
        ...
        <section class="bg-white p-5 rounded-xl shadow-sm">
            <form method="GET" action="{{ route('simpanan.transaksi') }}" class="flex flex-wrap gap-3 items-end">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="No. transaksi atau nama..." class="w-full border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary">
                    </div>
                </div>
                ...
                <a href="{{ route('simpanan.transaksi') }}" class="px-5 py-2.5 bg-slate-100 text-slate-600 text-sm font-semibold rounded-xl hover:bg-slate-200 transition-all">Reset</a>
            </form>
        </section>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @forelse($transaksi as $trx)
                            ...
                            <tr class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <p class="text-sm font-semibold text-blue-900">{{ $trx->rekening?->anggota?->nama_lengkap ?? '-' }}</p>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <span class="font-data font-bold text-sm {{ $isCredit ? 'text-secondary' : 'text-danger' }}">
